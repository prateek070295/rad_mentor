...
const LearnTab = ({ todayFocus }) => {
  ...
  const generatePersonalizedStudyMaterial = async (quizResults) => {
    ...
    try {
      ...
      let correctTopics = new Set();

      quizResults.forEach(response => {
        const topic = response.question.questionText.split(':')[0]; // Simple way to extract topic
        if (response.aiFeedback?.isCorrect) {
          correctTopics.add(topic);
        } else {
      ...
      const personalizedPrompt = `
        ...
        Here is the original study material for reference:
        ${studyMaterial}

        Here are the user's pre-test results and feedback:
        ${feedbackSummary}

        Based on these results, generate new, personalized study material for the user.
        - **Focus more deeply** on concepts related to questions the user answered **incorrectly** (topics: ${Array.from(incorrectTopics).join(', ') || 'None identified'}). Provide additional explanations, examples, or clarification for these areas.
        - **Briefly review** concepts related to questions the user answered **correctly** (topics: ${Array.from(correctTopics).join(', ') || 'None identified'}). Reiterate key points without extensive detail.
        - Structure the response as a coherent study guide, using clear headings and bullet points.
        - Ensure the tone is encouraging and educational.
        - The response should be plain text, suitable for direct display as study material.
      `;
