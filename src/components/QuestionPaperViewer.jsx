...
const QuestionPaperViewer = () => {
    ...
    useEffect(() => {
        if (year) {
            const matchingPapersForYear = papersList.filter(p => p.year === Number(year));
            
            // --- DEBUGGER LOGS ---
            console.log(`Filtering for year: ${year}`);
            console.log("Found matching paper documents:", matchingPapersForYear);
            // ---------------------

            const months = [...new Set(matchingPapersForYear.map(p => p.month))].filter(Boolean); // .filter(Boolean) removes any null/undefined
    ...
    useEffect(() => {
        if (month && year) {
            const papers = [...new Set(papersList.filter(p => p.year === Number(year) && p.month === month).map(p => p.paper))].sort();
    ...
    const handleProceed = (e) => { e.preventDefault(); setSelectedPaper({ exam, year: Number(year), month, paper }); setView('confirmation'); };
    ...
    const handleNextQuestion = () => { if (currentIndex < questions.length - 1) { setCurrentIndex(prev => prev + 1); setIsAnswerVisible(false); } };
    const handlePreviousQuestion = () => { if (currentIndex > 0) { setCurrentIndex(prev => prev - 1); setIsAnswerVisible(false); } };
    const handleRevealAnswer = () => setIsAnswerVisible(true);

    // --- Debugger Log ---
    console.log("--- 5. Preparing to render. Current state:", {
        view,
        isLoading,
        error,
    ...
    if (view === 'test_mode') {
        ...
        console.log("--- Rendering: Test Mode View ---");
        return ( <div><div className="bg-gray-100 p-4 rounded-lg mb-6 flex justify-between items-center"><h2 className="text-xl font-bold">Test in Progress: {selectedPaper.paper}</h2><div className="text-right"><div className={`text-2xl font-bold ${timeLeft < 600 ? 'text-red-500' : 'text-gray-800'}`}>{formatTime(timeLeft)}</div><div className="text-xs text-gray-600">Time Remaining</div></div></div><div className="space-y-4 mb-6">{questions.map((q) => ( <div key={q.id} className="p-4 border rounded-lg bg-white"><p className="font-semibold text-gray-800">{q.questionNumber}. {q.questionText}</p>{q.repeatCount > 1 && (<div className="mt-2 text-xs text-blue-600 bg-blue-50 p-2 rounded-md"><strong>Repeated {q.repeatCount} times.</strong></div>)}<div className="text-sm text-gray-500 mt-2 flex items-center gap-4"><span>Marks: <strong>{q.marks}</strong>{q.marksDistribution && <span className="text-gray-400 ml-1">({q.marksDistribution})</span>}</span><span>Topic: <strong>{q.topic}</strong></span></div></div>))}</div><div className="text-center"><button onClick={handleEndTest} className="px-8 py-3 bg-gray-700 text-white rounded-lg font-semibold hover:bg-gray-800">End Test</button></div></div>);
    }
    ...
    if (view === 'practice_mode') {
        ...
        console.log("--- Rendering: Practice Mode View ---");
        return (
            <div>
                 <div className="bg-gray-100 p-4 rounded-lg mb-6 flex justify-between items-center"><h2 className="text-xl font-bold">Practice Session: {selectedPaper.paper}</h2><button onClick={goBackToFilter} className="text-sm text-blue-600 hover:underline">&larr; Back to Selection</button></div>
                <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200">
                    <div className="flex justify-between items-center text-sm font-semibold text-gray-500 mb-4"><span>Question {currentQuestion.questionNumber} of {questions.length}</span><span>Marks: <strong>{currentQuestion.marks}</strong>{currentQuestion.marksDistribution && <span className="text-gray-400 ml-1">({currentQuestion.marksDistribution})</span>}</span></div>
                    <p className="text-xl font-semibold text-gray-800 mb-6 min-h-[100px]">{currentQuestion.questionText}</p>
                    {currentQuestion.repeatCount > 1 && (<div className="mb-4 text-xs text-blue-600 bg-blue-50 p-2 rounded-md"><strong>Repeated {currentQuestion.repeatCount} times.</strong></div>)}
                    {isAnswerVisible ? ( <div className="p-4 bg-green-50 rounded-lg border border-green-200"><p className="text-sm font-bold text-green-800">Topic:</p><p className="text-lg text-green-900">{currentQuestion.topic}</p></div>) : (<button onClick={handleRevealAnswer} className="w-full py-2 px-4 bg-blue-100 text-blue-700 font-semibold rounded-lg hover:bg-blue-200">Reveal Answer</button> )}
                </div>
                <div className="flex justify-between items-center mt-6"><button onClick={handlePreviousQuestion} disabled={currentIndex === 0} className="px-6 py-2 rounded-lg font-semibold transition-colors duration-200 bg-gray-200 text-gray-800 hover:bg-gray-300 disabled:opacity-50">&larr; Previous</button><button onClick={handleNextQuestion} disabled={currentIndex === questions.length - 1} className="px-6 py-2 rounded-lg font-semibold transition-colors duration-200 bg-gray-200 text-gray-800 hover:bg-gray-300 disabled:opacity-50">Next &rarr;</button></div>
            </div>
        );
    }
